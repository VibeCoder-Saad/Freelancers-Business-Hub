...
from database.database_manager import (get_all_projects_with_client_name, start_time_entry,
                                       ...
                                       delete_time_entry, log_activity)
from datetime import datetime, timedelta

# --- TRACKING IMPORTS ---
from tracking.screenshot_worker import ScreenshotWorker
...
class TimeTrackingView(QWidget):
    ...
    def toggle_timer(self):
        if self.live_timer.isActive():
            ...
            # Initialize & Start Trackers
            self.window_tracker = WindowTracker(project['id'])
            self.screenshot_thread = ScreenshotWorker(project['id'], interval_seconds=30) # DEMO: Every 30 secs
    ...
    def update_display_time(self):
        if self.timer_start_time:
            ...
            self.timer_label.setText(str(timedelta(seconds=seconds)))
            
            # Log Window Activity every 30 seconds
            if self.window_tracker and seconds % 30 == 0:
                log_data = self.window_tracker.log_current_window()
                log_activity(log_data['project_id'], log_data['window_title'])
