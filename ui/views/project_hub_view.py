...
class ProjectHubView(QWidget):
    def __init__(self, user_role="Freelancer", user_id=None):
        super().__init__()
        ...
        # --- RBAC: Hide Buttons for Client ---
        if self.user_role == "Client":
            self.delete_project_button.setVisible(False)
    ...
    def refresh_project_list(self):
        ...
        # --- RBAC: Filter Projects ---
        if self.user_role == "Client" and self.user_id:
            projects = self.controller.get_projects_for_client(self.user_id)
        else:
            projects = self.controller.get_all_projects()
    ...
    def display_project_dashboard(self):
        ...
        # Rebuild UI
        if self.dashboard_widget.layout(): 
            # Clear old layout items
            QWidget().setLayout(self.dashboard_widget.layout())
        ...
        header_layout.addWidget(title); header_layout.addWidget(client)
        layout.addLayout(header_layout)

        # Tabs: Overview | Chat
        dash_tabs = QTabWidget()
        layout.addWidget(dash_tabs)
        ...
        overview_layout.addWidget(time_header)
        time_table = self.create_time_table(data['time_entries']); overview_layout.addWidget(time_table)

        invoice_header = QLabel("Associated Invoices"); invoice_header.setObjectName("HeaderLabel")
        overview_layout.addWidget(invoice_header)
        invoice_table = self.create_invoice_table(data['invoices']); overview_layout.addWidget(invoice_table)
        
        dash_tabs.addTab(overview_widget, "Overview")
        
        # --- Tab 2: Chat ---
        # Pass user creds to ChatView
        chat_widget = ChatView(project_id, self.user_id, self.user_role)
        dash_tabs.addTab(chat_widget, "Live Chat")
